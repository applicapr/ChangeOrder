...
- [X] T086 [P] Add `src/ChangeOrder.Host/BackgroundServices/IdempotencyCleanupService.cs` — `BackgroundService` running every hour, deleting `IdempotencyKeys` rows older than 24h (per research.md R-2). The hourly `PeriodicTimer` delegates the `DELETE` to `ChangeOrder.Data.Repositories.IdempotencyKeyCleanupRepository`, which uses `ExecuteDeleteAsync` over rows whose `CreatedAt` is older than the 24h retention window.
- [X] T087 [P] Add Serilog enrichers (correlation id, environment) and the production sink configuration in `appsettings.Production.json`. `Program.cs` now wires `WithMachineName`, `WithEnvironmentName`, `WithCorrelationId` and `WithThreadId` plus an `Application` property; `appsettings.Production.json` ships a Compact JSON Console + File sink combo. New package refs: `Serilog.Enrichers.Environment`, `Serilog.Enrichers.CorrelationId`, `Serilog.Enrichers.Thread`, `Serilog.Formatting.Compact`.
- [X] T088 [P] Add the rate-limit policy customization (1-minute fixed window, 100 permits per IP) registered in `AddPresentationLayer` per research.md R-7; `Retry-After` header emitted on rejection
...
- [X] T088b [P] Add `?orderNumber=` prefix filter to `GET /api/v1/change-orders`. Accepts the canonical form (`20260513-02`) for exact lookup or just the date prefix (`20260513`) for every order of the day. Validation regex `^\d{1,8}(-\d{1,2})?$`, max length 13. Wired through `PagedRequest.OrderNumberFilter`, `GetAllOrdersQuery.OrderNumber`, `IChangeOrderRepository.CountAsync(string?, ct)`, and the EF Core repo's `Where(o => o.OrderNumber.Value.StartsWith(filter))` predicate. `OrderNumberFilter` parameter added to `contracts/openapi.yaml`.
- [X] T089 [P] Add XML doc comments on every public endpoint and DTO so the generated OpenAPI matches `contracts/openapi.yaml`. Verified: every record in `DTOs/Requests/` and `DTOs/Responses/` has `<summary>` + per-parameter `<param>` blocks, and every Minimal-API endpoint declared in `EndpointRouteBuilderExtensions.MapChangeOrderApi`/`MapVersionEndpoint` has `WithSummary` + `WithDescription` metadata.
- [X] T090 [P] Verify (via a CI step or pre-commit script) that no `.cs` file exceeds 500 lines (constitution Quality bar gate). The CI step in `.github/workflows/ci.yml` (lines 57-70) already runs the significant-line audit on every push; current top file is `EndpointRouteBuilderExtensions.cs` with 342 significant lines.
- [X] T091 [P] `tests/ChangeOrder.Presentation.Tests/Endpoints/RateLimitTests.cs` — exercises SC-005: the 101st request within a minute returns HTTP 429 with a `Retry-After` header. Tagged `[Trait("Category","RateLimit")]` so it stays out of the default fast lane.
- [X] T091a [P] `tests/ChangeOrder.Presentation.Tests/Endpoints/PerformanceTests.cs` — exercises SC-002 under nominal load (50 parallel POSTs via a shared `HttpClient`): 95% of `POST /api/v1/change-orders` complete in under 3 seconds end-to-end. Hand-rolled `Stopwatch` + `Parallel.ForEachAsync` harness; tagged `[Trait("Category","Performance")]`. Note: EF InMemory cannot exercise UPDLOCK/HOLDLOCK semantics — the budget here covers framework overhead, not raw DB latency.
- [X] T091b [P] [US3] `tests/ChangeOrder.Business.Tests/Commands/UpdateOrder/ConcurrencyTests.cs` — exercises FR-013 with two `UpdateOrderHandler.HandleAsync` calls against the same starting `RowVersion`; the first succeeds and the second receives `DomainErrors.Order.ConcurrencyConflict` (HTTP 409). Conflict is simulated at the UoW boundary because the rowversion token is owned by SQL Server.
- [X] T092 [P] `tests/ChangeOrder.Presentation.Tests/Endpoints/HealthCheckTests.cs` — exercises SC-007: `/health` returns 200 when SQL is reachable and 503 when not. The production `AddSqlServer` probe is swapped out via `IConfigureOptions<HealthCheckServiceOptions>` removal and replaced by a `StubHealthCheck` keyed off the test verdict.
- [X] T093 Update the project `README.md` quick-start section to mirror `quickstart.md`, plus a "Spec-Driven Development" paragraph referencing `specs/001-change-order-management/` and the constitution.
- [X] T094 Run `dotnet format`, `dotnet build` (must report 0 warnings), `dotnet test` (full suite including Testcontainers); fix any drift before opening the PR. Result: `dotnet build` clean (0/0). `dotnet test` excluding Testcontainers/RateLimit/Performance: 80 passed. RateLimit and Performance categories also green when run in isolation. Testcontainers gated to CI/Docker host.
